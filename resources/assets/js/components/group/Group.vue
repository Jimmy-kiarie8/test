<template>
  ...
  <div>
              ...
              <div class="col-xs-12">
                <div class="box">
                  <div class="box-header">
                    <h3 class="box-title">List Of all Users groups</h3>
    ...
    <groupAdd @closeRequest="close" :openAddRequest="dispAdd" @alertRequest="snackbartrue"></groupAdd>
    <groupShow  @closeRequest="close" :openShowRequest="dispShow"></groupShow>
    <groupUpdate @closeRequest="close" :openRequest="dispUpdate" @alertRequest="snackbartrue"></groupUpdate>
  <!-- </v-app> -->
</div>
</transition>
...
let groupAdd = require('./Addgroup.vue')
let groupShow = require('./showgroup.vue')
let groupUpdate = require('./Updategroup.vue')
// let mynew = require('../Newhead.vue')
export default{
  props: ['user'],
  components: {
    groupShow, groupUpdate, groupAdd, vueTopprogress
    // , mynew
  },
  ...
  methods: {
    ...
    openShow(key){
      this.$children[3].list = this.group[key]
      this.dispShow = 'block'
    },
    openUpdate(key){
      this.$children[4].list = this.group[key]
  ...
  mounted() {
    ...
    axios.post('/getgroup')
    .then((response) => this.group = this.temp = response.data)
      .catch((error) => this.errors = error.response.data.errors)
  },
}
</script>
